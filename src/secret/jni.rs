...
use crate::jni_bridge::helpers::load_app_class;
...
impl SecretStore for JniSecretStore {
    fn get(&self, alias: &str) -> Result<SecretBytes, SecretError> {
        ...
        #[cfg(target_os = "android")]
        let class = load_app_class(&mut env, BRIDGE_CLASS)
            .map_err(|e| SecretError::Backend(format!("load_app_class: {e}")))?;
        #[cfg(not(target_os = "android"))]
        let class = env
            .find_class(BRIDGE_CLASS)
            .map_err(|e| SecretError::Backend(format!("find_class: {e}")))?;

        let alias_j: JString = env
            .new_string(alias)
        ...
        let bytes_obj = res
            .l()
            .map_err(|e| SecretError::Backend(format!("not an object: {e}")))?;
        if bytes_obj.is_null() {
            return Err(SecretError::NotFound(alias.to_owned()));
        }
        let bytes_j: JByteArray = bytes_obj.into();
        let len = env
            .get_array_length(&bytes_j)
            .map_err(|e| SecretError::Backend(format!("get_array_length: {e}")))?
            as usize;
