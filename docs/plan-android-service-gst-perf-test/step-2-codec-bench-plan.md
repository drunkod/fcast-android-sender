# Step 2 — `src/codec_bench_plan.rs` (new): JSON plan

← [Step 1](step-1-codec-perf-entrypoints.md) · [Index](README.md) · Next → [Step 3](step-3-codec-bench-jni.md)

`serde`/`serde_json` are already deps — no `Cargo.toml` change. This module is the
process-agnostic “what to run” layer; the service process drives it via JNI (Step 3).
...
impl CodecBenchResponse {
    pub fn ok(report: String, ran_decode: bool, should_kill_process: bool) -> Self {
        Self { ok: true, report, ran_decode, should_kill_process, error: None }
    }
    pub fn error(error: impl Into<String>) -> Self {
        Self { ok: false, report: String::new(), ran_decode: false, should_kill_process: false, error: Some(error.into()) }
    }
}
...
pub fn run_benchmark_plan(request: CodecBenchRequest) -> CodecBenchResponse {
    ...
    if request.include_decode_perf {
        // Optional Step 9 EGL guard — keep the binding alive across decode.
        #[cfg(feature = "foreign-egl")]
        let _egl_guard = request
            .use_foreign_egl
            .then(crate::codec_egl::try_install_foreign_egl_for_current_process);
...
> The `#[cfg(feature = "foreign-egl")]` block compiles away unless you do Step 9 and
> enable that cargo feature. Without it, the plan still works — the **separate
> process** is the real protection; the guard is extra hygiene.
