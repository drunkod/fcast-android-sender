# Step 4 — `src/jni_bridge/codec_test.rs` (new)

← [Step 3](step-3-codec-test-page-slint.md) · [Index](README.md) · Next → [Step 5](step-5-mod-rs.md)

Follows the `vm()` + `attach_current_thread` + `load_app_class` + `call_static_method`
pattern from `src/secret/jni.rs`. `helpers::vm()` is `pub(crate)` and returns
`Arc<JavaVM>`; `load_app_class` resolves app dex classes from native-attached threads
where `env.find_class()` can't.
