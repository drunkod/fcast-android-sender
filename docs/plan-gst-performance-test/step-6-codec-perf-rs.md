# Step 6 — `src/codec_perf.rs` (new)

← [Step 5](step-5-settings-entry.md) · [Index](README.md) · Next → [Step 7](step-7-lib-rs-register.md)

New file. Pure-Rust GStreamer benchmark — no Kotlin/JNI. Verified against the
tree: `gst = "gstreamer" 0.25`, `androidmedia` is in `app/jni/Android.mk`, and
`crate::platform::gst_init::ensure_gstreamer_initialized() -> Result<(), String>`
exists (it is `pub(crate)`, callable from this same-crate module).
...
> Two locals are assigned but unused in the research source (`registry` in
> `list_codec_factories`, and `got_eos`, `warn` import). Expect `unused`
> warnings — either prefix with `_` (`let _registry = …`, `let mut _got_eos`) or
> drop them. They don't block compilation. See Step 9.
